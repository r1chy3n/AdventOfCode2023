...
fun main() {
    ...
    val configurations = contraption.indices.map { index ->
        Pair( Pair( 0, index ), right )
    }.toMutableList()

    configurations.addAll(
        contraption.indices.map { index ->
            Pair( Pair( contraption[ index ].lastIndex, index ), left )
        } // map
    ) // addAll

    configurations.addAll(
        contraption.first().indices.map { index ->
            Pair( Pair( index, 0 ), down )
        } // map
    ) // addAll

    configurations.addAll(
        contraption.last().indices.map { index ->
            Pair( Pair( index, contraption.lastIndex ), up )
        } // map
    ) // addAll

    println(
        configurations.maxOf { entrance ->
            ...
            val beams = mutableListOf( entrance )
